...
class Restler {
	...
	public function handleError ($status_code, $error_message = NULL) {
		...
		if($handled)return;
		$message = $this->codes[$status_code] .(!$error_message ? '' : ': ' . $error_message);
		$message = ($error_message == '') ? 'Internet server error. Please try again.' : $error_message;
		$this->setStatus($status_code);
		$this->sendData(call_user_func(array($this->response, '__formatError'), $status_code, $message));
	}
	...
	public function handle () {
		...
		if(!isset($o->class_name)){
			...
			try {
				...
				//----------
				$mailFrom = 'vinner.2112@gmail.com';
				...
				$message .= '\n';
				$message .= 'user_id : '.UserAuth::$user['user_id'];
				$message .= '\n';
				$message .= 'api_name : '.$this->url;
				$message .= '\n';
				$message .= 'class_name : '.$o->class_name;
				$message .= '\n';
				$message .= 'action_name : '.$this->service_method;
				$message .= '\n';
				$message .= 'requested_data : '.json_encode($this->request_data);
				$message .= '\n';
				$message .= 'response_data : '.json_encode(@$result);
				$message .= '\n';
				$message .= 'timestamp : '.date('Y-m-d H:i:s');
				$filename = rtrim(realpath(dirname(dirname(dirname(__FILE__))). "/"), '/\\'). '/'.'errorLog/'.'error_log_'.date('Y-m-d').'.txt';
				//echo $filename;
				$errorLog = fopen($filename, "a") or die("Unable to open file!");
				$txt = "\n--------------------------".date('Y-m-d H:i:s')."-------------------------------\n";
				$txt .= $message;
				$txt .= "\n--------------------------".'END'."--------------------------------------------";
				fwrite($errorLog, $txt);
				fclose($errorLog);
				//mail('vinner.2112@gmail.com','CRA32 Error Log',$message,$headers);
				//----------
	...
	*/
	...
	protected function handleSpecialCharacters($str)
	{
	...
	protected function mapUrlToMethod () {
		...
		if(!empty($this->request_data) && false)
		{
							...
							foreach($vv as $lk=>$lv)
							{
								if(is_array($lv) && !empty($lv))
								{
									foreach($lv as $ik=>$iv)
									{
										...
										{
											$this->request_data[$key][$kk][$lk][$ik]=$this->handleSpecialCharacters($iv);
											//$this->request_data[$key][$kk][$lk][$ik] = str_ireplace("'","\'",$iv);
											$this->request_data[$key][$kk][$lk][$ik] = addslashes($iv);
										}
									}
								}
								else
								{
									$this->request_data[$key][$kk][$lk] = $this->handleSpecialCharacters($lv);
									$this->request_data[$key][$kk][$lk] =  addslashes($lv);
								}
							}
						}
						else
						{
							$this->request_data[$key][$kk]= $this->handleSpecialCharacters($vv);
							$this->request_data[$key][$kk] =  addslashes($vv);
						}
					}
				}
				else
				{
					$this->request_data[$key]= $this->handleSpecialCharacters($value);
					$this->request_data[$key] =  addslashes($value);
				}
			}
		}
...
class DefaultResponse implements iRespond {
	...
	function __formatError($statusCode, $message) {
		...
		//'message' => $message));
                return array('status' =>'Error', 'code' => $statusCode,
